                    ...
                    if (this == draggingSlot.slot){
                        ...
                        if (this.identifier != selectedSlot.identifier) {
                            if (this.type == "combining") {
                                ...
                                this.parentElement.childNodes[this.itemIndex].style.backgroundImage = `url(assets/inventory/${this.item.model}.png)`;
                                ...
                                if (firstSlot.item.model && secondSlot.item.model) {
                                    ...
                                    outputSlot.style.backgroundImage = `url(assets/inventory/${firstSlot.item.model}.png)`;
...
function createContextMenu(ele) {
    ...
    if (match != null) {
        ...
        if (attachments != null){
            ...
            attachmentMenuContainer.style.width = "150px";
...
function constructTooltip(slot,type,item) {
    ...
    if (item.attachments) {
        for (var key in item.attachments) {
            ...
            attachment.innerHTML = `<b>${key}</b>-${item.attachments[key]}`;
